  ...
  return (
    <div className="overflow-y-auto p-2 flex flex-col gap-0.5">
      {data.map((d) =>
        d.rooms.map((room) => (
          <PageLink
            key={room.roomId || room.pageId}
            room={room}
            active={pathname === `/${room.pageId ?? room.info?.pageId ?? ""}`}
          />
        ))
      )}
...
function PageLink({ room, active }: { room: any; active: boolean }) {
  return (
    <div
    ...
    >
      <Link href={room.info.url} className="py-1.5 px-3 flex-1 truncate">
        <LiveTitle
          pageId={room.pageId}
          fallback={room.info.name || "Untitled document"}
        />
      </Link>
    </div>
  );
